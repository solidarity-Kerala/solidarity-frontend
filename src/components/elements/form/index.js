              ...
              formState.map((item, index) => {
                ...
                return (props.formType === "put" && item.update) || props.formType === "post" ? (
                  item.type === "multiple" && item.forms ? (
                    ...
                    <FormInput dynamicClass={dynamicClass} animation={`sub-1`} placeholder={item.placeHolder} key={`input` + index} id={index} error={formErrors[formState[index].name]} value={formValues[formState[index].name]} {...item} onChange={handleChange} />
                  )
                ) : (
                  ""
                );
