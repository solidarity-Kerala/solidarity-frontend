...
const AvailableCaloriesCustom = ({ openData, themeColors, setMessage }) => {
  ...
  const [, setCurrentRecipe] = useState();
  const [, setSelectedIngredient] = useState();
  const [, setSelectedRecipeVariants] = useState();
  ...
  const deleteItem = async (
    ...
    setMessage({
      ...
      onProceed: async () => {
        ...
        await getData({ meal: openData?.data?._id }, "meal-item").then(
          (response) => {
  ...
  const onDrop = async (item, data) => {
    ...
    await getData(
      ...
      setSelectedRecipeVariants(response?.data?.response);
    });
  };
                ...
                {questions?.map((variant) => (
                  ...
                  <DraggableItem
                    ...
                    element={
                      <MealItem
                        key={variant?._id}
                        onClick={() => onDrop(variant)}
                      >
                        <Title>
                          {variant?.recipe?.title ?? "Title not found!"} {"-"}{" "}
                          {variant?.variant}
                        </Title>
                        <Title>
                          {/* <span>BHD</span>
